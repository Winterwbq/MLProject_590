...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run dedicated SUPER RATE (CC/S) analysis on parsed NERS590 CSV tables."
    )
    parser.add_argument(
        "--parsed-dir",
        type=Path,
    ...
    args.parsed_dir = normalize_v03_analysis_path(args.parsed_dir, "parsed", log_prefix="[super-analysis]")
    args.output_dir = normalize_v03_analysis_path(
        args.output_dir, "super_rate_analysis", log_prefix="[super-analysis]"
    )
    ...
    overall_row = {
        ...
        **numeric_summary(super_values, include_negative=True),
        ...
        by_power_rows.append(
            {
                ...
                **numeric_summary(values, include_negative=True),
