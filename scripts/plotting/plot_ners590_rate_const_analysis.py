...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create summary figures for NERS590 RATE CONST analysis outputs."
    )
    parser.add_argument(
        "--analysis-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "results" / "ners590_v03_analysis" / "figures",
        help="Directory for figure outputs.",
    )
    args = parser.parse_args()
    args.analysis_dir = normalize_v03_analysis_path(args.analysis_dir, "analysis", log_prefix="[rate-plot]")
    args.output_dir = normalize_v03_analysis_path(args.output_dir, "figures", log_prefix="[rate-plot]")
    ...
    ax.set_xlabel("Power")
    ax.set_ylabel("Cases")
    save_figure(fig, args.output_dir / "case_count_by_power.png")
    ...
    save_figure(fig, args.output_dir / "e_over_n_grid_by_power.png")
    ...
    save_figure(fig, args.output_dir / "input_species_mean_heatmap_by_power.png")
    ...
    save_figure(fig, args.output_dir / "median_rate_by_power.png")
    ...
    save_figure(fig, args.output_dir / "electron_energy_vs_power.png")
    ...
    save_figure(fig, args.output_dir / "top_power_sensitive_reactions.png")
    ...
    ax.set_ylabel("Reaction")
    save_figure(fig, args.output_dir / "top_reactions_by_mean_rate.png")
